...
アプリケーションコンテナのことをDocker, Inc. は「A standardized unit of software (ソフトウェアの標準化された単位)」だと表現している．
...
- Container Runtime: コンテナの実行や状態管理を行うシステム
    - `containerd`: 高レベルコンテナランタイム, CRI Runtime
    - `runc`: 低レベルコンテナランタイム, OCI Runtime
